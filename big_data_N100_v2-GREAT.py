...
r = requests.get('http://cdn-hk.evilucifero.com/wiki/NASDAQ-100')
...
link_adj = []
for link in links:
    link_temp = 'http://cdn-hk.evilucifero.com' + link
    r = requests.get(link_temp)
    soup = BeautifulSoup(r.content, 'html.parser')
    infobox = soup.find('table', {"class": "infobox"})
    temp = infobox.find('th', text = 'Industry')
    if temp != None:
        link_adj.append(link)



import numpy as np
links_unique, link_counts = np.unique(link_adj, 
                                      return_counts=True)
